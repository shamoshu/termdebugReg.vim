...
let s:register_names = []
let s:register_values = {}
let s:prev_register_values = {}
let s:reg_matches = []
let s:show_float = v:false

let s:prev_mem_values = {}
let s:mem_matches = []
let s:current_mem_addr  = ''
...
" highlight link debugRegisterChanged DiffChange
" highlight default link debugRegisterChanged DiffText
" highlight link debugRegisterChanged Identifier
" highlight link debugRegisterChanged Type
highlight link debugRegisterChanged Statement
...
func s:EndDebugCommon()
  ...
  if s:regbufnr > 0 && bufexists(s:regbufnr)
    exe $'bwipe! {s:regbufnr}'
  endif
  if s:membufnr > 0 && bufexists(s:membufnr)
    exe $'bwipe! {s:membufnr}'
  endif
...
function! s:HandleRegisterNames(msg)
  let names = matchstr(a:msg, 'register-names=\zs\[.*\]\ze')
  let s:register_names = eval(names)
endfunction

function! s:RequestRegisterNames()
  if empty(s:register_names)
    call s:SendCommand('-data-list-register-names')
  endif
endfunction

function! s:IsFloatRegister(name)
  return a:name =~ '^[sd]\d\+$'
endfunction


function! s:HandleRegisterValues(msg)
  let curwinid = win_getid()

  if win_gotoid(s:regwin)

    " alte Matches löschen
    for id in s:reg_matches
      call matchdelete(id)
    endfor
    let s:reg_matches = []

    silent! %delete _
    call setline(1, 'Register        Value(hex)     Value(dec)')
    ...
    let capture = '{number="\d\+",value="[^"]\+"}'
    let reginfo = matchstr(a:msg, capture, 0, cnt)

    while reginfo != ''

      let number = matchstr(reginfo, 'number="\zs\d\+\ze"')
      let value  = matchstr(reginfo, 'value="\zs[^"]\+\ze"')

      let idx = str2nr(number)

      if idx < len(s:register_names)
        let name = s:register_names[idx]
      else
        let name = 'r'.idx
      endif
    
      if !s:show_float && s:IsFloatRegister(name)
        let cnt += 1
        let reginfo = matchstr(a:msg, capture, 0, cnt)
        continue
      endif
      ...
      " 👉 speichern
      let new_values[name] = value
 
      let clean = matchstr(value, '^0x[0-9a-fA-F]\+')

      if clean != ''
        let dec = str2nr(clean, 16)
        if strlen(clean) > 10
          " 64-bit
          if dec >= 0x8000000000000000
            let dec = dec - 0x10000000000000000
          endif
        else
          " 32-bit
          if dec >= 0x80000000
            let dec = dec - 0x100000000
          endif
        endif
      else
        let dec = ''
      ...
      " 👉 rendern (richtige Zeile!)
      call setline(line_nr,
        \ printf("%-16s%-14s %s", name, value, dec))

      " 👉 DIREKT vergleichen (wichtig!)
      if has_key(s:prev_register_values, name)
        if s:prev_register_values[name] != value
          let id = matchadd('debugRegisterChanged', '\%' . line_nr . 'l\s\+\zs.*')
          call add(s:reg_matches, id)
        endif
      endif
    ...
    let s:prev_register_values = new_values
...
function! s:MemoryScroll(delta)
  " Hex-String in Zahl umwandeln
  let l:addr_num = str2nr(s:current_mem_addr, 16)
  
  " Adresse berechnen
  let l:addr_num += a:delta
  
  " Untergrenze prüfen (nicht unter 0)
  if l:addr_num < 0 | let l:addr_num = 0 | endif
  
  " Zurück in Hex-String umwandeln und speichern
  let s:current_mem_addr = printf("0x%08x", l:addr_num)
  
  " Display aktualisieren
  call s:UpdateMemoryDisplay()
endfunction



function! s:UpdateMemoryDisplay()
  " 1. Fenster anspringen, um die Höhe zu messen
  let l:curwinid = win_getid()
  if !win_gotoid(s:memwin)
    return
  endif

  " 2. Höhe berechnen (Fensterhöhe minus 1 Zeile Header)
  let l:rows = winheight(0) - 1
  if l:rows < 1 | let l:rows = 1 | endif
  
  " 3. Bytes berechnen (16 Bytes pro Zeile)
  let l:byte_count = l:rows * 16

  " 4. GDB Befehl senden
  let l:cmd = printf('-data-read-memory-bytes %s %d', s:current_mem_addr, l:byte_count)
  call s:SendCommand(l:cmd)

  " Zurück zum Ursprungsfenster
  call win_gotoid(l:curwinid)
endfunction

function! s:HandleMemBytes(msg)
 let curwinid = win_getid()
  
  if win_gotoid(s:memwin)
    for id in s:mem_matches | call matchdelete(id) | endfor
    let s:mem_matches = []

    let start_addr_hex = matchstr(a:msg, 'begin="\zs[^"]\+\ze"')
    let contents = matchstr(a:msg, 'contents="\zs[^"]\+\ze"')
    if contents == '' | return | endif

    silent! %delete _
    call setline(1, printf('Basis-Adresse: %s | J/K: Scrollen | C-F/B: Seite', s:current_mem_addr))
    call setline(2, 'Adresse:     Memory                                Text')
    

    let start_addr = str2nr(start_addr_hex, 16)
    let new_values = {}
    let line_nr = 3
    
    let i = 0
    while i < strlen(contents)
      let line_hex = strpart(contents, i, 32)
      let current_addr = start_addr + (i / 2)
      let addr_str = printf("0x%08x", current_addr)
      
      let new_values[addr_str] = line_hex
      
      " Hex-Blöcke formatieren
      let m1 = strpart(line_hex, 0, 8)
      let m2 = strpart(line_hex, 8, 8)
      let m3 = strpart(line_hex, 16, 8)
      let m4 = strpart(line_hex, 24, 8)
      let mem_display = printf("%-8s %-8s %-8s %-8s", m1, m2, m3, m4)

      " ASCII-Teil generieren
      let text_display = ""
      for j in range(0, strlen(line_hex) - 1, 2)
        let byte = str2nr(strpart(line_hex, j, 2), 16)
        let text_display .= (byte >= 32 && byte <= 126) ? nr2char(byte) : "."
      endfor

      call setline(line_nr, printf("%s:  %s  %s", addr_str, mem_display, text_display))

      " --- PRÄZISES HIGHLIGHTING ---
      if has_key(s:prev_mem_values, addr_str)
        let old_line = s:prev_mem_values[addr_str]
        
        " Prüfe jedes der 16 Bytes einzeln
        for b_idx in range(0, 15)
          let new_byte = strpart(line_hex, b_idx * 2, 2)
          let old_byte = strpart(old_line, b_idx * 2, 2)
          
          if new_byte != old_byte
            " Berechnung der Startspalte im Vim-Fenster:
            " Adresse (10) + ":  " (3) = 13 Zeichen Vorlauf
            " Jedes Byte braucht 2 Zeichen. 
            " Alle 4 Bytes (8 Zeichen) kommt ein Leerzeichen hinzu.
            let spaces = b_idx / 4
            let col_start = 14 + (b_idx * 2) + spaces
            
            " Highlight nur für dieses Byte (2 Zeichen breit)
            let id = matchaddpos('debugRegisterChanged', [[line_nr, col_start, 2]])
            call add(s:mem_matches, id)
          endif
        endfor
      endif
      " -----------------------------
    ...
    let s:prev_mem_values = new_values
  endif
  call win_gotoid(curwinid)
endfunction
  ...
  for msg in a:msgs
    ...
    elseif msg != ''
      if msg =~ '^\(\*stopped\|\*running\|=thread-selected\)'
        ...
        if msg =~ '^\*stopped'
          call s:SendCommand('-data-list-register-values x')
        endif
        if msg =~ '^\*stopped'
          call s:UpdateMemoryDisplay()
        endif
      ...
      elseif msg =~ '^\^done,register-values='
        call s:HandleRegisterValues(msg)
      elseif msg =~ '^\^done,register-names='
        call s:HandleRegisterNames(msg)
      elseif msg =~ '^\^done,memory='
        call s:HandleMemBytes(msg)
      endif
    endif
  endfor
endfunc
...
function! s:OpenMemoryWithAddr(addr)
  " 1. Adresse speichern (und sicherstellen, dass sie mit 0x beginnt)
  let s:current_mem_addr = a:addr
  if s:current_mem_addr !~ '^0x'
    let s:current_mem_addr = '0x' . s:current_mem_addr
  endif

  " 2. Fenster öffnen/anspringen
  call s:GotoMemorywinOrCreateIt()

  " 3. Daten sofort anfordern
  call s:UpdateMemoryDisplay()
endfunction
...
func s:InstallCommands()
  ...
  command Reg call s:GotoRegisterwinOrCreateIt()
  command Winbar call s:InstallWinbar(1)
  " -nargs=1: Erwartet genau ein Argument
  command! -nargs=1 Mem call s:OpenMemoryWithAddr(<q-args>)
...
function! s:GotoRegisterwinOrCreateIt()
  if !win_gotoid(s:regwin)
    let mdf = ''
    if win_gotoid(s:sourcewin)
      if winwidth(0) > (78 + 60)
    ...
    if s:regbufnr > 0 && bufexists(s:regbufnr)
      exe $'buffer {s:regbufnr}'
    elseif empty(glob('Termdebug-registers'))
      silent file Termdebug-registers
      let s:regbufnr = bufnr('Termdebug-registers')
    else
      call s:Echoerr("File 'Termdebug-registers' exists already")
    endif
  endif

  call setline(1, 'Loading registers...')
...
function! s:SetupMemoryMappings()
  " Scrollen mit Pfeiltasten oder J/K
  nnoremap <buffer> <silent> j :call <SID>MemoryScroll(16)<CR>
  nnoremap <buffer> <silent> k :call <SID>MemoryScroll(-16)<CR>
  nnoremap <buffer> <silent> <C-f> :call <SID>MemoryScroll(64)<CR>
  nnoremap <buffer> <silent> <C-b> :call <SID>MemoryScroll(-64)<CR>

  " Mausrad-Unterstützung
  nnoremap <buffer> <silent> <ScrollWheelUp>   :call <SID>MemoryScroll(-16)<CR>
  nnoremap <buffer> <silent> <ScrollWheelDown> :call <SID>MemoryScroll(16)<CR>
endfunction

function! s:GotoMemorywinOrCreateIt()
  if !win_gotoid(s:memwin)
    ...
    if s:membufnr > 0 && bufexists(s:membufnr)
      exe $'buffer {s:membufnr}'
    elseif empty(glob('Termdebug-memory'))
      silent file Termdebug-memory
      let s:membufnr = bufnr('Termdebug-memory')
    else
      call s:Echoerr("File 'Termdebug-memory' exists already")
    endif
  endif

  " setlocal nomodifiable
  call s:SetupMemoryMappings() " Hier die Mappings setz
  call setline(1, 'Loading memory...')
